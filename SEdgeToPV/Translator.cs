...
namespace SEdgeToPV
{
    internal class Translator : IDisposable
    {
        ...
        public void Execute()
        {
            TranslateInfo TransalteInfo = TranslateInfo.createFromInputFile(InFile);
            log.DebugFormat("Created translation info {0}", TransalteInfo.ToString());

            //Ensure output directory exists
            if (!Directory.Exists(TransalteInfo.ConversionOutputDir))
            {
                Directory.CreateDirectory(TransalteInfo.ConversionOutputDir);
            }

            bool PrimaryFileConversionResult = false;
            if (string.Equals(TransalteInfo.FileType, "dft", StringComparison.CurrentCultureIgnoreCase))
            {
                PrimaryFileConversionResult = Convert2DModel(TransalteInfo);
            }
            else
            {
                PrimaryFileConversionResult = Convert3DModel(TransalteInfo);
            }


            if (PrimaryFileConversionResult && TransalteInfo.AdditionalFileFormats.Count > 0)
            {
                IList<string> CreatedAdditionalFiles = CreateAdditionalFileFormats(TransalteInfo);
                ...
                PackFilesToPvoa(TransalteInfo, TransalteInfo.FileName, CreatedAdditionalFiles);
            }
            ...
            {
                File.WriteAllText(string.Format("{0}\\loaderoptions.txt", TransalteInfo.ConversionOutputDir), "thumbnailcreate=true\n");
            }

        }

        private IList<string> CreateAdditionalFileFormats(TranslateInfo TransalteInfo)
        {
            IList<string> CreatedAdditionalFiles = new List<string>();
            string InputFile = TransalteInfo.FileName;
            foreach (string FileFormat in TransalteInfo.AdditionalFileFormats)
            {

                string ResultFile = string.Format("{0}\\{1}.{2}", TransalteInfo.ConversionOutputDir, InputFile, FileFormat);
                ...
                ExternalExecutableResult ConversionResult = PerformSolidEdgeTranslation(TransalteInfo, IntermediateFormat);
                ...
                if (Directory.GetFiles(TransalteInfo.ConversionOutputDir, string.Format("*.{0}", FileFormat)).Length < 1)
                {
                ...
                string[] GeneratedOutputFiles = Directory.GetFiles(TransalteInfo.ConversionOutputDir, string.Format("*.{0}", FileFormat));
                ...
                {
                    string PackedFilePath = PackFilesToZip(TransalteInfo, InputFile, FileFormat, GeneratedOutputFiles);
                    CreatedAdditionalFiles.Add(PackedFilePath);
                }

                CleanUpLogs(TransalteInfo);
            }
            return CreatedAdditionalFiles;
        }

        private string PackFilesToZip(TranslateInfo TransalteInfo, string InputFile, string PackedExtension, IEnumerable<string> GeneratedOutputFiles)
        {
            string WorkingDir = Directory.CreateDirectory(string.Format("{0}\\{1}", TransalteInfo.ConversionOutputDir, Guid.NewGuid())).FullName;
            ...
            string ZipPath = string.Format("{0}\\{1}_{2}.zip", TransalteInfo.ConversionOutputDir, InputFile, PackedExtension);
        ...
        private string PackFilesToPvoa(TranslateInfo TransalteInfo, string InputFile, IEnumerable<string> GeneratedOutputFiles)
        {
            string WorkingDir = Directory.CreateDirectory(string.Format("{0}\\{1}", TransalteInfo.ConversionOutputDir, Guid.NewGuid())).FullName;
            ...
            foreach (string GeneratedOutputFile in GeneratedOutputFiles)
            {
                ...
                if (GeneratedOutputFile.ToLower().EndsWith(".pdf") && string.Equals(TransalteInfo.FileType, "dft", StringComparison.CurrentCultureIgnoreCase))
                {
            ...
            string ZipPath = string.Format("{0}\\{1}.zip", TransalteInfo.ConversionOutputDir, Guid.NewGuid());
            ZipFile.CreateFromDirectory(WorkingDir, ZipPath);

            File.Move(ZipPath, string.Format("{0}\\additionals.pvoa", TransalteInfo.ConversionOutputDir, InputFile));
        ...
        private bool Convert2DModel(TranslateInfo TransalteInfo)
        {
            try
            {
                ExternalExecutableResult ConversionResult = PerformSolidEdgeTranslation(TransalteInfo, IntermediateFormat.PDF);
                ...
                string ASCIIPVSPath = CrateASCIIPVSFile(TransalteInfo, ConversionResult);
                if (!ConversionResult.Result || !File.Exists(ConversionResult.ResultFile))
                {
                ...
                ExternalExecutableResult PVSResult = PerfomASCIIPVSToBinaryConversion(ASCIIPVSPath, TransalteInfo);
        ...
        private static string CrateASCIIPVSFile(TranslateInfo TransalteInfo, ExternalExecutableResult ConversionResult)
        {
            string ASCIIPVSPath = string.Format("{0}\\{1}_ascii.pvs", TransalteInfo.ConversionOutputDir, TransalteInfo.FileName);
            ...
            using (XmlWriter writer = XmlWriter.Create(ASCIIPVSPath, settings))
            {
                ...
                //Section structure
                {
                    ...
                    writer.WriteAttributeString("name", TransalteInfo.FileName);
        ...
        private bool Convert3DModel(TranslateInfo TransalteInfo)
        {
            try
            {
                ...
                ExternalExecutableResult IntermediateResult = PerformSolidEdgeTranslation(TransalteInfo, IntermediateFormat);
                ...
                ExternalExecutableResult PVSResult = PerformPVSConversion(IntermediateResult.ResultFile, TransalteInfo, IntermediateFormat);
                log.Debug("PVS File Generation Result");
                log.DebugFormat("Executable return code: {0}", PVSResult.ExecutableExitCode);
                log.DebugFormat("Executable STDOUT: {0}", PVSResult.ExecutableStdOut);
                log.DebugFormat("Executable STDERR: {0}", PVSResult.ExecutableStdErr);
        ...
        private static void CleanUpLogs(TranslateInfo TransalteInfo)
        {
            //CleanUp Intermediate Log Files
            foreach (string LogFile in Directory.GetFiles(TransalteInfo.ConversionOutputDir, "*.log"))
            {
                File.Delete(LogFile);
            }
        }
